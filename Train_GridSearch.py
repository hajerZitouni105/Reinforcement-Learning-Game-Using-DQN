...
class ModifiedTensorBoard(TensorBoard):
    ...
    def _train_writer(self):
        return self.writer
    @_train_writer.setter
    def _train_writer(self, value):
        self.writer = value
    ...
    def on_train_begin(self, logs=None):
        # Ensure the writer and training step are properly initialized
        self._train_writer = self.writer
        self._train_step = self.step

    def set_model(self, model):
        # No need to set model explicitly since we override how the logs are handled
        pass

    def on_epoch_end(self, epoch, logs=None):
        # Override to save stats at the end of each epoch
        self.update_stats(**logs)

    def on_batch_end(self, batch, logs=None):
        # Optionally, handle logging after every batch if needed
        pass

    def on_train_end(self, _):
        # Prevent Keras from closing the writer
        pass
...
def save_model_and_weights(agent, model_name, episode, max_reward, average_reward, min_reward):
    ...
    checkpoint_name = re.sub(r'[<>:"/\\|?*]', '', checkpoint_name)
    checkpoint_name = checkpoint_name.replace(' ', '_')
    agent.model.save(f'{PATH}models/{checkpoint_name}.keras')
    print(f"Model saved as {checkpoint_name}.keras")
...
for i, m in enumerate(models_arch):
    ...
    res = res._append({"Model Name":MODEL_NAME, "Convolution Layers":m["conv_list"], "Dense Layers":m["dense_list"],
    ...
    # Remove invalid characters from MODEL_NAME
    safe_model_name = MODEL_NAME.replace("|", "_").replace(":", "_")
    avg_df.to_csv(f"{PATH}results/{safe_model_name}-Results-Avg.csv")
    max_df.to_csv(f"{PATH}results/{safe_model_name}-Results-Max.csv")
